...
public class NewsController extends AbstractController {
  ...
  protected void doGet(HttpServletRequest req,
    ...
    if (items == null) {
      ...
      Pagination pagination = new PaginationBuilder(requestUrl + "?", offset, items.getCount())
          .withLimit(Constant.LIMIT_ARTICLES_PER_PAGE).build();
  ...
  private Items<Article> listArticles(String requestUrl, int offset, HttpServletRequest req) {
    ...
    if (requestUrl.endsWith("/news") || requestUrl.endsWith("/news/")) {
      ...
      items = getBusinessService()
          .listArticlesByCategory(categoryUrl, offset, Constant.LIMIT_ARTICLES_PER_PAGE);
